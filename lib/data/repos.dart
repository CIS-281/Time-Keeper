...
class JobSiteRepo {
  ...
  Future<List<JobSite>> all(String companyId) async {
    ...
    return rows.map(JobSite.fromMap).toList();
  }
...
class ClockRepo {
  ...
  Future<List<ClockEvent>> inRangeUtc(
      int empId,
      int startUtc,
      int endUtc, {
    ...
    return rows.map(ClockEvent.fromMap).toList();
  }
}
...
class ShiftRepo {
  ...
  Future<void> endShift(
    ...
    await db.update(
      'shifts',
      {'clock_out_utc': nowSec, 'status': 'clocked_out'},
      where: 'id=? AND company_id=?',
      whereArgs: [shiftId, companyId],
    );
  ...
  Future<List<Shift>> recentShifts(
      int employeeId, {
        required String companyId,
        int limit = 20,
      }) async {
  ...
  Future<List<ShiftSummary>> summariesForEmployee(
      int employeeId, {
        required String companyId,
        int? fromUtc,
        int? toUtc,
    ...
    return rows.map(ShiftSummary.fromMap).toList();
  }
}
